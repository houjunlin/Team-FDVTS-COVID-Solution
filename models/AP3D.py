...
class APM(nn.Module):
    ...
    def forward(self, x):
        ...
        # neighbor_time_index = torch.tensor([1,1,2,2]).t().flatten().long()

        # neighbor_time_index = torch.tensor([1,0]).t().flatten().long()
        ...
        # x_norm_expand = x_norm.permute(0, 2, 3, 4, 1).contiguous().view(-1, h*w, c//16) # (b*t, h*w, c/16) 
        # neighbor_norm = x_norm[:, :, neighbor_time_index, :, :].permute(0, 2, 1, 3, 4).contiguous().view(-1, c//16, h*w) # (b*t, c/16, h*w) 
        ...
        # neighbor = x[:, :, neighbor_time_index, :, :].permute(0, 2, 3, 4, 1).contiguous().view(-1, h*w, c)
        # neighbor_new = torch.matmul(similarity, neighbor).view(b, t, h, w, c).permute(0, 4, 1, 2, 3) # (b, c, t, h, w)

        # contrastive attention
        if self.contrastive_att:
            x_att = self.x_mapping(x.unsqueeze(3).expand(-1, -1, -1, N-1, -1, -1).contiguous().view(b, c, (N-1)*t, h, w).detach())
            # x_att = self.x_mapping(x.contiguous().view(b, c, t, h, w).detach())
        ...
        # x_offset = torch.zeros([b, c, 2*t, h, w], dtype=x.data.dtype, device=x.device.type)
        # x_index = torch.tensor([i for i in range(t*2) if i%2==2//2])
        # neighbor_index = torch.tensor([i for i in range(t*2) if i%2!=2//2])
        # x_offset[:, :, x_index, :, :] += x
        # x_offset[:, :, neighbor_index, :, :] += neighbor_new
