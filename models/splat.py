...
from torch.nn import Conv2d, Conv3d, Module, Linear, BatchNorm2d, ReLU
from torch.nn.modules.utils import _pair

from models import inflate

__all__ = ['SplAtConv2d', 'DropBlock2D', 'SplAtConv3d', 'DropBlock3D']
...
class SplAtConv3d(Module):
    """Split-Attention Conv2d
    """
    def __init__(self, SplAtConv2d, in_channels, channels, kernel_size, stride=(1, 1, 1), padding=(0, 0, 0),
        ...
        if self.rectify:
            ...
            #                     groups=groups*radix, bias=bias, average_mode=rectify_avg, **kwargs)
            self.conv = inflate.inflate_conv(SplAtConv2d.conv)
        else:
        ...
        if self.use_bn:
            # self.bn0 = norm_layer(channels*radix)
            self.bn0 = inflate.inflate_batch_norm(SplAtConv2d.bn0)
        self.relu = ReLU(inplace=True)
        # self.fc1 = Conv3d(channels, inter_channels, 1, groups=self.cardinality)
        self.fc1 = inflate.inflate_conv(SplAtConv2d.fc1)
        if self.use_bn:
            # self.bn1 = norm_layer(inter_channels)
            self.bn1 = inflate.inflate_batch_norm(SplAtConv2d.bn1)
        # self.fc2 = Conv3d(inter_channels, channels*radix, 1, groups=self.cardinality)
        self.fc2 = inflate.inflate_conv(SplAtConv2d.fc2)
        if dropblock_prob > 0.0:
